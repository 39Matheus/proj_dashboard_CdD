...
def render_tab3(df_1, data_inicial, data_final):
    ...
    for grau, cor, poly, model in zip(graus_lista, cores, polys_treinados, modelos_treinados):
        # Reaproveita os modelos treinados na tabela
        ...
        fig.add_trace(go.Scatter(
            ...
            y=y_all,
            mode='lines',
            name=f'Regressão Grau {grau} (Histórico + Projeção)',
    ...
    # Mapeia grau -> modelo já treinado (caso exista)
    modelos_map = {grau: (modelos_treinados[i], polys_treinados[i]) for i, grau in enumerate(graus_lista)}

    for grau in graus:
        if grau in modelos_map:
            model, poly = modelos_map[grau]
        else:
            poly = PolynomialFeatures(degree=grau)
            poly.fit(x_train)  # fit só precisa do X
